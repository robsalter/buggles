...
var cb = function (d, c, hide) {
  ...
  for (x in d) {
    y = d[x];
    s = y.duration/300;
    ipLnk = y.programme.is_available_mediaset_pc_sd ? '<a href="http://www.bbc.co.uk/iplayer/episode/' + y.programme.pid +'">watch</a>' : '';
    dt = new DateObj(y.start);
    h += '<div class="m-group"><article class="' + c + '-' + x +'"><dl><dt>' + y.programme.display_titles.title + '</dt>';
    h += '<dd class="p-time"><span class="hour">' + dt.hh + '</span>:<span class="minutes">' + dt.mm + '</span></dd>';
    h += '<dd class="p-details"><ul><li class="replay">' + ipLnk + '</li></ul></dd></dl></article></div>';
    for (i = 1; i <= s; i++) {
